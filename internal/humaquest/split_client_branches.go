...
//ff:what Splits response branches into client (<500, PASS-gated) and advisory server (>=500, shown but not gating) sets.
...
// (Status >= 500, kept for visibility but never silently dropped). Dropping 5xx
// would shrink the denominator and become a cheese enabler (§3.6). Ported from
// bak/cmd/split_client_branches.go.
